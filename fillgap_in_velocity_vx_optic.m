...
close all;
format long;
path='/home/zez/JI/optic_velocity/vx_xyz/';
path3='/home/zez/JI/optic_velocity/filled_vx/';
%%%%%%%%%%
%index1 points in polygon with value
%index2 points in polygon without value
%nan for vx TSX is -2000000000
...
for u=1:length(file)
    ...
    u
    file(u).name
    save temp_file_vx file path u path3;
    clear all;
    load temp_file_vx;
    data=load([path file(u).name]);
    
name=[path3 file(u).name(1:25) '.filled.txt' ];
path2='/home/zez/JI/contour_fillgap/';
polygon_file=dir([path2 'contour_fill_gap_second_xy.txt']); 
count_for_polygon=0;
for h=1:length(polygon_file)
    save temp_file_vx2 file path u name path2 path3 polygon_file h data count_for_polygon;
    clear all;
    load temp_file_vx2;
...
elseif(m==(count+1))
    ...
    commond=['cp ' [path file(u).name] ' ' name ]
    system(commond);
    continue;
else
    k=length(index2);
